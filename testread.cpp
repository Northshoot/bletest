...
void randomNum(void)
{
	...
	Serial.print(rnd);
	Serial.println("\n");
	lib_aci_open_remote_pipe(&aci_state, PIPE_LARS_SERVICE_RANDOMSUM_RX);
}
  ...
  if (lib_aci_event_get(&aci_state, &aci_data))
  {
    ...
    switch(aci_evt->evt_opcode)
    {
      ...
      case ACI_EVT_PIPE_STATUS:
        ...
        if (lib_aci_is_pipe_available(&aci_state, PIPE_LARS_SERVICE_RANDOMSUM_RX_REQ)
             )
        {

        	Serial.print(F("PIPE_LARS_SERVICE_RANDOMSUM_RX_REQ"));
        } else if (lib_aci_is_pipe_available(&aci_state, PIPE_LARS_SERVICE_RANDOMSUM_RX)
                 )
            {
        	Serial.print(F("PIPE_LARS_SERVICE_RANDOMSUM_RX"));
            } else {
            	if(true ==lib_aci_open_remote_pipe(&aci_state, PIPE_LARS_SERVICE_RANDOMSUM_RX_REQ)){
            		Serial.println(F("PIPE_LARS_SERVICE_RANDOMSUM_RX_REQ true"));
            	} else if(true ==lib_aci_open_remote_pipe(&aci_state, PIPE_LARS_SERVICE_RANDOMSUM_RX)){
            		Serial.println(F("PIPE_LARS_SERVICE_RANDOMSUM_RX true"));
            	} else {
            		Serial.println(F("org"));
            	}
